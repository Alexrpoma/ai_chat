            ...
            try:
                ...
                if result and result.final_output:
                    ...
                    try:
                        ...
                        # We extract the value of the 'text' key, which is another String Json
                        if isinstance(first_level_data, dict) and 'text' in first_level_data:
                            json_string_from_text_key = first_level_data['text']

                            # The interior string becomes the final dictionary
                            ...
                            if isinstance(second_level_data, dict) and 'html' in second_level_data:
                                ...
                                # If the second JSON does not have 'html', we use the string as failback
                                final_response_text = json_string_from_text_key
                        else:
                            # If the first JSON does not have 'text', we use the original string as failure
                ...
                if result and result.raw_responses:
                    llm_response = result.raw_responses[0]
                    if llm_response.usage:
                        self.current_context_tokens = llm_response.usage.input_tokens
                        print(
                            f"API: Tokens usage: {llm_response.usage.input_tokens} input, {llm_response.usage.output_tokens} output.")

                assistant_message = {"role": "assistant", "content": final_response_text}
                print(f"Alva: {final_response_text}")
